...
struct FlickData
{
  typedef enum
  {
    ...
    AutoScrollAcceleration // Scrolling automatically but a finger is on the
                           // screen
  } State;
  State state;
  QWidget* widget;
  ...
  void updateSpeed(const QPoint& newPosition)
  {
    if (lastPosValid) {
      ...
      if (timeElapsed) {
        ...
        // stopping the autoscroll because of a small horizontal offset
        ...
        if (state == AutoScrollAcceleration) {
          ...
          if (!speed.isNull()) {
            speed.setX(
              qBound(-max, (speed.x() / 4) + (newSpeedX * 3 / 4), max));
            speed.setY(
              qBound(-max, (speed.y() / 4) + (newSpeedY * 3 / 4), max));
          } else {
...
void
FlickCharm::activateOn(QWidget* widget)
{
...
void
FlickCharm::deactivateFrom(QWidget* widget)
{
  QAbstractScrollArea* scrollArea = qobject_cast<QAbstractScrollArea*>(widget);
  if (scrollArea) {
    QWidget* viewport = scrollArea->viewport();
...
static QPoint
deaccelerate(const QPoint& speed, const int deltatime)
{
  ...
  int y = speed.y();
  x =
    (x == 0) ? x : (x > 0) ? qMax(0, x - deltaSpeed) : qMin(0, x + deltaSpeed);
  y =
    (y == 0) ? y : (y > 0) ? qMax(0, y - deltaSpeed) : qMin(0, y + deltaSpeed);
  return QPoint(x, y);
}

bool
FlickCharm::eventFilter(QObject* object, QEvent* event)
{
...
void
FlickCharm::timerEvent(QTimerEvent* event)
{
  int count = 0;
  ...
  while (item.hasNext()) {
    ...
    if (data->state == FlickData::AutoScrollAcceleration &&
        data->waitingAcceleration && data->accelerationTimer.elapsed() > 40) {
      data->state = FlickData::ManualScroll;
      data->resetSpeed();
    }
